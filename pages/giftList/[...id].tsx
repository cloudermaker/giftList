...
const GiftPage = ({ user, giftList }: { user: User; giftList: Gift[] }) => {
                ...
                <Suspense fallback="loading...">
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                        <SortableContext items={localGifts}>
                            {localGifts
                                // filter if filteringTakenGifts is true
                                .filter((gift) =>
                                    filteringTakenGifts ? gift.takenUserId == null || gift.userId === connectedUser?.userId : true
                                )
                                .map((gift, idx) => (
                                    <SortableItem key={`gift_${gift.id}`} gift={gift} idx={idx + 1} canReorder={userCanAddGift}>
                                        <GiftItem
                                            ...
                                            userCanAddGift={userCanAddGift}
                                            isEditing={updatingGiftId === gift.id}
                                            name={updatingGiftId === gift.id ? newGiftName : gift.name}
                                            description={updatingGiftId === gift.id ? newDescription : (gift.description ?? '')}
                                            link={updatingGiftId === gift.id ? newLink : (gift.url ?? '')}
                                            error={updatingGiftId === gift.id ? error : ''}
                                            onEdit={() => updatingGift(gift)}
                                            onRemove={() => removeGift(gift.id)}
                                            onUpsert={upsertGift}
                                            onCancelEdit={clearAllFields}
                                            onBlockUnBlockGift={() => onBlockUnBlockGiftClick(gift)}
                                            setName={setNewGiftName}
...
export async function getServerSideProps(context: any) {
    ...
    return {
        props: {
            ...
            giftList: giftList.map((gift: any) => ({
                ...gift,
